...
class Election {
  ...
  make_new_vote_select(name) {
    return $('<select>').attr('id', this.id + name + "_new_vote").attr('name', name + "_new_vote");
  }
  ...
  refill_vote_select(name) {
    ...
    for (let candidate of this.state.candidates) {
      v.append($('<option>').attr('value', candidate.name).html(candidate.name));
    }
  }
  ...
  build(desc) {
    ...
    $('#' + this.id + 'num_rounds').val(num_rounds); // need to take a list of candidates so we
...
function mouseCtrl(n, getCtrl, setCtrl) {
  ...
  ctrl = document.getElementById(n); // on mousedown start tracking mouse relative position
...
exports.seq_phragmen = exports.Assignment = exports.Candidate = exports.Voter = exports.Vote = void 0;
...
const seq_phragmen = (assignment, num_to_elect) => {
  ...
  for (let round = 0; round < num_to_elect; round++) {
    ...
    for (let voter of assignment.voters) {
      for (let vote of voter.votes) {
        let candidate = assignment.find_candidate(vote.can_name); // assume it exists...
